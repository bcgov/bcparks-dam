...
readarray -t directories < <(find /var/www/resourcespace/filestore/system/ -type d -name "slideshow_*" -printf '%T+ %p\n' | sort | cut -d' ' -f2-)
...
sudo sed -i "s|'filestore/system/slideshow_[^']*'|'filestore/system/$directory_name'|" /var/www/resourcespace/include/config.php
